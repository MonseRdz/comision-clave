...
BEGIN
  IF OLD.estatus = 'Aprobado' AND NOT public.es_contralor() THEN
    IF NEW.estatus IS DISTINCT FROM OLD.estatus
       ...
       -- Solo se admite mover el avance del incremento (clave "incremento");
       -- monto, responsable, fecha compromiso y cierres del saldo son del Contralor.
       OR (COALESCE(NEW.documentacion, '{}'::jsonb) - 'incremento')
          IS DISTINCT FROM (COALESCE(OLD.documentacion, '{}'::jsonb) - 'incremento')
    THEN
...
REVOKE EXECUTE ON FUNCTION public.proteger_gasto_aprobado() FROM PUBLIC, anon, authenticated;

-- El revisor también necesita ver y mover el incremento de un gasto aprobado con saldo abierto.
...
USING (
  ...
  OR ((estatus = 'Aprobado'::text) AND ((documentacion ->> 'estatus'::text) = 'Abierto'::text)
      AND ((comisionado_id = auth.uid()) OR ((documentacion ->> 'responsableId'::text) = (auth.uid())::text)))
)
WITH CHECK (
  es_contralor()
  OR has_role(auth.uid(), 'Revisor'::app_role)
